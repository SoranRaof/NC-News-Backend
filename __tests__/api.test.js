  ...
  test("responds with an array of user objects", () => {
    return request(app)
      ...
      .then((res) => {
        ...
        expect(res.body.users).not.toBeEmpty();
        expect(res.body.users.length).toBe(4);
        for (let i = 0; i < res.body.users.length; i++) {
          expect(res.body.users[i]).toBeInstanceOf(Object);
          expect(res.body.users[i]).not.toBeEmpty();
          expect(res.body.users[i]).toMatchObject({
...
describe("DELETE /api/comments/:comment_id", () => {
  ...
  test("comment is deleted from database", () => {
    return request(app)
